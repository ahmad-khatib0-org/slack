...
func generateHooksGlue(info *PluginInterfaceInfo) {
	templateFunctions := map[string]any{
		"funcStyle": func(fields *ast.FieldList) string {
			return FieldListToFuncList(fields, info.FileSet)
		},
		"structStyle": func(fields *ast.FieldList) string {
			return FieldListToStructList(fields, info.FileSet)
		},
		"valuesOnly": func(fields *ast.FieldList) string {
			return FieldListToNames(fields, false)
		},
	...
	if err := os.WriteFile(filepath.Join(getPluginPackageDir(), "client_rpc_generated.go"), formatted, 0o664); err != nil {
		panic(err)
	}
}
...
func generatePluginTimerLayer(info *PluginInterfaceInfo) {
	templateFunctions := map[string]any{
		...
		"valuesOnly": func(fields *ast.FieldList) string {
			return FieldListToNames(fields, true)
		},
		"destruct": func(structPrefix string, fields *ast.FieldList) string {
			return FieldListDestruct(structPrefix, fields, info.FileSet)
		},
	...
	for fileName, presetTemplate := range pluginTemplates {
		...
		if err := os.WriteFile(filepath.Join(getPluginPackageDir(), fileName), formatted, 0o664); err != nil {
			panic(err)
		}
	}
}

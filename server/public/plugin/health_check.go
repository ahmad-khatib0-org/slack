...
// CheckPlugin determines the plugin's health status, then handles the error or
// success case. If the plugin passes the health check, do nothing. If the plugin
// fails the health check, the function either restarts or deactivates the plugin,
// based on the quantity and frequency of its failures.
...
// shouldDeactivatePlugin determines if a plugin needs to be deactivated after the
// plugin has failed (HealthCheckNumRestartsLimit) times, within the configured time
// window (HealthCheckDeactivationWindow).

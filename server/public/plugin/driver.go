...
// ResultContainer contains the output from the LastInsertID and RowsAffected methods
// for a given set of rows. It is used to embed another round-trip to the server,
...
// Driver is a sql driver interface that is used by plugins to perform raw SQL queries
// without opening DB connections by themselves. This interface is not subject to
// backward compatibility guarantees and is only meant to be used by plugins built
// by the Mattermost team.
type Driver interface {
	...
	// Note: the following cannot be implemented because either MySQL or PG does not support
	// it. So this implementation has to be a common subset of both DB implementations.
...
type AppDriver interface {
	...
	// This is an extra method needed to shutdown connections after a plugin shuts down.

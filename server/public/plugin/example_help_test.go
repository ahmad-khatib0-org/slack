...
// configuration represents the configuration for this plugin as
//
// exposed via the Mattermost server configuration.
...
// HelpPlugin implements the interface expected by the Mattermost server
...
// concurrently. The active configuration may change underneath the client of this method,
// but the struct returned by this API call is considered immutable.
...
//
// Do not call setConfiguration while holding the configurationLock,
// as sync.Mutex is not reentrant.
...
func (p *HelpPlugin) MessageHasBeenPosted(c *plugin.Context, post *model.Post) {
	...
	p.API.SendEphemeralPost(post.UserId, &model.Post{
		...
		Props:     map[string]any{"sent_by_plugin": true},
	})
}

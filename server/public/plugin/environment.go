...
import (
	...
	"github.com/ahmad-khatib0-org/slack/server/public/utils"
...
//
// It is meant for use by the Mattermost server to manipulate, interact with
// and report on the set of active plugins.
...
//
// This function will return info for all subdirectories that appear to be plugins
// (i.e. all subdirectories containing plugin manifest files, regardless of whether
// they could actually be parsed).
//
...
// TransitionallyPrepackagedPlugins returns a list of plugins transitionally
// prepackaged in the local prepackaged_plugins folder.
//
// The list content is immutable and should not be modified.
...
// ClearTransitionallyPrepackagedPlugins clears the list of plugins
// transitionally prepackaged in the local prepackaged_plugins folder.
...
// RunMultiPluginHook invokes hookRunnerFunc for each active plugin that implements
// the given hookId. If hookRunnerFunc returns false, iteration will not continue.
// The iteration order among active plugins is not specified.
...
// TogglePluginHealthCheckJob starts a new job if one is not running and is set to enabled,
// or kills an existing one if set to disabled.

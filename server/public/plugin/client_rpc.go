...
// ErrorString is a fallback for sending unregistered implementations of the error
// interface across rpc. For example, the errorString type from the github.com/pkg/errors
// package cannot be registered since it is not exported, but this precludes common error
// handling paradigms. ErrorString merely preserves the string description of the error,
// while satisfying the error interface itself to allow other registered types
// (such as model.AppError) to be sent unmodified.
...
func (s *hooksRPCServer) OnActivate(args *Z_OnActivateArgs, returns *Z_OnActivateReturns) error {
	...
	dbClient := &dbRPCClient{client: rpc.NewClient(conn2)}
	...
	// Capture output of standard logger because go-plugin redirects it.
...
// Using a subset of http.Request prevents a known incompatibility when decoding
// Go v1.23+ gob-encoded x509.Certificate structs from Go v1.22 compiled plugins.
// These come from http.Request.TLS field (*tls.ConnectionState).
...
func (g *apiRPCClient) PluginHTTP(request *http.Request) *http.Response {
	...
	_args := &Z_PluginHTTPArgs{Request: forwardedRequest}
...
// MessageWillBePosted is in this file because of the difficulty of identifying which
// fields need special behaviour. The special behaviour needed is decoding the returned
...
// MessageWillBeUpdated is in this file because of the difficulty of identifying which
// fields need special behaviour. The special behaviour needed is decoding the returned
// post into the original one to avoid the unintentional removal of fields by older plugins.
func init() {
...
// MessagesWillBeConsumed is in this file because of the difficulty of identifying which
// fields need special behaviour. The special behaviour needed is decoding the returned
// post into the original one to avoid the unintentional removal of fields by older plugins.
func init() {

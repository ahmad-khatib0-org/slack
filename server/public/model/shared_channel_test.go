...
func TestSharedChannelIsValid(t *testing.T) {
	...
	data := []struct {
		...
		{name: "Invalid share_name", sc: &SharedChannel{
			ChannelId: id, TeamId: id, CreateAt: now, UpdateAt: now,
			ShareName: "@test@",
		}, valid: false},
		{name: "Too long share_name", sc: &SharedChannel{
			ChannelId: id, TeamId: id, CreateAt: now, UpdateAt: now,
			ShareName: strings.Repeat("01234567890", 100),
		}, valid: false},
		{name: "Missing creator_id", sc: &SharedChannel{
			ChannelId: id, TeamId: id, CreateAt: now, UpdateAt: now,
			ShareName: "test",
		}, valid: false},
		{name: "Missing remote_id", sc: &SharedChannel{
			ChannelId: id, TeamId: id, CreateAt: now, UpdateAt: now,
			ShareName: "test", CreatorId: id,
		}, valid: false},
		{name: "Valid shared channel", sc: &SharedChannel{
			ChannelId: id, TeamId: id, CreateAt: now, UpdateAt: now,
			ShareName: "test", CreatorId: id, RemoteId: id,
		}, valid: true},
	}

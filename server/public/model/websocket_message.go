...
type WebsocketBroadcast struct {
	...
	// BroadcastHooks is a slice of hooks IDs used to process events before sending them
	// on individual connections. The IDs should be understood by the WebSocket code.
	//
	// This field should never be sent to the client.
	BroadcastHooks []string `json:"broadcast_hooks,omitempty"`
	// BroadcastHookArgs is a slice of named arguments for each hook invocation.
	// The index of each entry corresponds to the index of a hook ID in BroadcastHooks
	//
	// This field should never be sent to the client.
...
// WithoutBroadcastHooks gets the broadcast hook information from a WebSocketEvent and
// returns the event without that. If the event has broadcast hooks, a copy of the event
// is returned. Otherwise, the original event is returned. This is intended to be called
// before the event is sent to the client.
...
func NewWebSocketEvent(
	event WebsocketEventType,
	teamId, channelId, userId string,
	omitUsers map[string]bool,
	omitConnectionId string,
) *WebSocketEvent {

...
//
// The 'access' tag '*_read' checks for any Sysconsole read permission and grants
// access if any read permission is allowed.
//
// By default config values can be written with PermissionManageSystem, but if
// ExperimentalSettings.RestrictSystemAdmin is true and the access tag contains the
// value 'write_restrictable', then even PermissionManageSystem, does not grant write access
// unless the request is made using local mode.
//
// PermissionManageSystem always grants read access.
//
// Config values with the access tag 'cloud_restrictable' mean that are marked to be
// filtered when it's used in a cloud licensed environment with
// ExperimentalSettings.RestrictedSystemAdmin set to true.
//
// Example:
//
//		type HairSettings struct {
//		    // Colour is writeable with either PermissionSysconsoleWriteReporting or
//	     // PermissionSysconsoleWriteUserManagementGroups. It is readable by
//	     // PermissionSysconsoleReadReporting and PermissionSysconsoleReadUserManagementGroups
//	     // permissions. PermissionManageSystem grants read and write access.

...
// Treats content between " as a single word (e.g., "hello world" → one word).
// Supports hyphen-prefixed quotes (e.g., -"hello" → - and "hello" as separate words).
// Edge Cases:
// Mixed quoted/non-quoted text (e.g., foo "bar baz" qux → ["foo", "\"bar baz\"", "qux"]).
// Hyphens before quotes (e.g., -"-flag" → ["-", "\"-flag\""]).
	...
	for i, char := range text {
		if char == '"' {
			if foundQuote {
				...
				nextStart := i
				// Without i > 0, the function would crash on inputs starting with ".
				if i > 0 && text[i-1] == '-' {
					nextStart = i - 1
				}
...
// Format: <flag>:<value> or -<flag>:<value> (for exclusion).
// Example: author:Jane → Flag author with value Jane.
// Example: -tag:bug → Exclude results with tag=bug.
//
// ### **Input/Output Examples**
// | Input (Slice)                     | Output (`words`)                    | Output (`flags`)                          |
// |-----------------------------------|-------------------------------------|-------------------------------------------|
// | `["hello", "author:John"]`        | `[{word: "hello", exclude: false}]` | `[{name: "author", value: "John", exclude: false}]` |
// | `["-tag:bug", "world"]`           | `[{word: "world", exclude: false}]` | `[{name: "tag", value: "bug", exclude: true}]` |
// | `["title:", "Go", "-discarded"]`  | `[]`                                | `[{name: "title", value: "Go", exclude: false}]` |
// | `["#foo", "-bar"]`                | `[{word: "#foo", exclude: false}, {word: "bar", exclude: true}]` | `[]` |

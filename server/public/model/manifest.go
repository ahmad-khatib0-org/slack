...
type PluginSetting struct {
	...
	//
	// "dropdown" will result in a string setting that allows the user to select
	// from a list of pre-defined options.
	//
	// "generated" will result in a string setting that is set to a random,
	// cryptographically secure string.
	//
	// "radio" will result in a string setting that allows the user to select from a
	// short selection of pre-defined options.
	//
	...
	//
	// "custom" will result in a custom defined setting and will load the custom
	//
	// component registered for the Web App System Console.
	...
	// For "radio" or "dropdown" settings, this is the list of pre-defined options
	// that the user can choose from.
	Options []*PluginOption `json:"options,omitempty" yaml:"options,omitempty"`

	// The intended hosting environment for this plugin setting. Can be "cloud" or "on-prem".
	// When this field is set, and the opposite environment is running the plugin, the setting
	// will be hidden in the admin console UI. Note that this functionality is entirely
	// client-side, so the plugin needs to handle the case of invalid submissions.
	Hosting string `json:"hosting"`

	// If true, the setting is sanitized before showing it in the System Console or
	// returning it via the API. This is useful for settings that contain sensitive information.
...
type PluginSettingsSection struct {
	...
	// If true, the section will load the custom component registered using
	// `registry.registerAdminConsoleCustomSection`
	Custom bool `json:"custom" yaml:"custom"`

	// If true and Custom = true, the settings defined under this section will still
	// render as fallback (unless the individual setting is type 'custom') when the
	// plugin is disabled.
...
// The manifest file should be named plugin.json or plugin.yaml and placed in the
// top of your plugin bundle.
//
// Example plugin.json:
//
//	{
...
type Manifest struct {
	// The id is a globally unique identifier that represents your plugin. Ids must
	// be at least 3 characters, at most 190 characters and must match ^[a-zA-Z0-9-_\.]+$.
	...
	// A relative file path in the bundle that points to the plugins svg icon for use
	// with the Plugin Marketplace. This should be relative to the root of your bundle
	// and the location of the manifest file. Bitmap image formats are not supported.
...
type ManifestWebapp struct {
	// The path to your webapp bundle. This should be relative to the root
	// of your bundle and the location of the manifest file.
...
//
// In all cases other than a does-not-exist error, path is set to the path of the
// manifest file that was found.
//

...
func TestUserMentionMap_ToURLValues(t *testing.T) {
	...
	for _, data := range fixture {
		...
		// require.EqualValues does not work here directly on the url.Values, as the slices
		// in the map values may be in different order; what we need to check is that the
		// pairs are preserved, which can be checked converting back to a map with FromURLValues.
		// We check that the test is well-formed by converting back the expected url.Values too.

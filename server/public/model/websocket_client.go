...
// WebSocketClient stores the necessary information required to communicate with
// a WebSocket endpoint. A client must read from PingTimeoutChannel, EventChannel
// and ResponseChannel to prevent deadlocks from occurring in the program.
...
// Connect creates a websocket connection with the given ConnectURL. This is racy and
// error-prone should not be used. Use any of the New* functions to create a websocket.
func (wsc *WebSocketClient) Connect() *AppError {
	return wsc.ConnectWithDialer(websocket.DefaultDialer)
}

// ConnectWithDialer creates a websocket connection with the given ConnectURL using the
// dialer. This is racy and error-prone and should not be used. Use any of the New*
// functions to create a websocket.
...
// Close closes the websocket client. It is recommended that a closed client
// should not be reused again. Rather a new client should be created anew.
...
func (wsc *WebSocketClient) Listen() {
	...
	//
	// First, we close some channels and then CAS to 1 and proceed to close the writer
	// chan also. This is needed because then the defer clause does not double-close
	// the writer when (2) happens.
...
func (wsc *WebSocketClient) GetStatusesByIds(userIds []string) {
	data := map[string]any{"user_ids": userIds}
...
func (wsc *WebSocketClient) UpdateActiveTeam(teamID string) {
	data := map[string]any{"team_id": teamID}
	wsc.SendMessage(string(WebsocketPresenceIndicator), data)
}

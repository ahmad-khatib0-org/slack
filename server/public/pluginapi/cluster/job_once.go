...
type JobOnce struct {
	...
	// join is a join point for the job.run() goroutine to join the
	// calling goroutine (in this case, the one calling job.Cancel)
	join     chan bool
	joinOnce sync.Once
...
func (j *JobOnce) run() {
	...
	for {
		...
		func() {
			...
			// If key doesn't exist, or if the runAt has changed, the original
			// job has been completed already
...
// readMetadata reads the job's stored metadata. If the caller wishes to make
// an atomic read/write, the cluster mutex for job's key should be held.
...
// saveMetadata writes the job's metadata to the kvstore. saveMetadata acquires
// the job's cluster lock. saveMetadata will not overwrite an existing key.

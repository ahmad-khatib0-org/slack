...
// GetJobOnceScheduler returns a scheduler which is ready to have
// its callback set. Repeated calls will return the same scheduler.
...
// Start starts the Scheduler. It finds all previous ScheduleOnce jobs and starts them
// running, and fires any jobs that have reached or exceeded their runAt time. Thus, even
// if a cluster goes down and is restarted, Start will restart previously scheduled jobs.
...
// SetCallback sets the scheduler's callback. When a job fires,
// the callback will be called with the job's id.
...
// ListScheduledJobs returns a list of the jobs in the db that have been scheduled.
// There is no guarantee that list is accurate by the time the caller reads the list.
// E.g., the jobs in the list may have been run, canceled, or new jobs may have scheduled.
...
// ScheduleOnce creates a scheduled job that will run once. When the clock reaches
// runAt, the callback will be called with key and props as the argument.
//
// If the job key already exists in the db, this will return an error.
// To reschedule a job, first cancel the original then schedule it again.
...
// Cancel cancels a job by its key. This is useful if the plugin lost the
// original *JobOnce, or is stopping a job found in ListScheduledJobs().

...
const (
	// mutexPrefix is used to namespace key values created for a
	// mutex from other key values created by a plugin.
	mutexPrefix = "mutex_"
)
...
// plugin API.
//
// Mutexes with different names are unrelated. Mutexes with the same name from different
// plugins are unrelated. Pick a unique name for each mutex your plugin requires.
//
...
type Mutex struct {
	...
	// lock guards the variables used to manage the refresh task,
	// and is not itself related to the cluster-wide lock.
	lock        sync.Mutex
...
func NewMutex(pluginAPI MutexPluginAPI, key string) (*Mutex, error) {
	...
	return &Mutex{pluginAPI: pluginAPI, key: key}, nil
}
...
// Lock locks m. If the mutex is already locked by any plugin instance, including
// the current one, the calling goroutine blocks until the mutex can be locked.
func (m *Mutex) Lock() {
	_ = m.LockWithContext(context.Background())
}

// LockWithContext locks m unless the context is canceled. If the mutex is already locked
// by any plugin instance, including the current one, the calling goroutine blocks until
// the mutex can be locked, or the context is canceled.
//
...
//
// Just like sync.Mutex, a locked Lock is not associated with a particular goroutine
// or plugin instance. It is allowed for one goroutine or plugin instance to lock a
// Lock and then arrange for another goroutine or plugin instance to unlock it. In
// practice, ownership of the lock should remain within a single plugin instance.
func (m *Mutex) Unlock() {
	m.lock.Lock()

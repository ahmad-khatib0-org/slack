...
const (
	// cronPrefix is used to namespace key values created for a job
	// from other key values created by a plugin.
	cronPrefix = "cron_"
)
...
// MakeWaitForInterval creates a function to scheduling a job to run on the
// given interval relative to the last finished timestamp.
//
...
// MakeWaitForRoundedInterval creates a function, scheduling a job to run on the
// nearest rounded interval relative to the last finished timestamp.
//
// For example, if the job first starts at 12:04 PM, and is configured with interval
// 5 minutes, and is configured to round to 5 minute intervals, it will next run at:
//
//	12:05 PM, 12:10 PM, 12:15 PM, ...
//
// If the job has not previously started, it will run immediately. Note that this wait
// interval strategy does not guarantee a minimum interval between runs, only that
// subsequent runs will be scheduled on the rounded interval.
...
// Job is a scheduled job whose callback function is executed on a configured interval
// by at most one plugin instance at a time.
//
...
// run attempts to run the scheduled job, guaranteeing
// only one instance is executing concurrently.
func (j *Job) run() {
	defer close(j.done)

...
// HTTPService wraps the functionality for making http requests to
// provide some improvements to the default client behaviour.
type HTTPService interface {
	// MakeClient returns an http client constructed with a RoundTripper as
	// returned by MakeTransport.
	MakeClient(trustURLs bool) *http.Client

	// MakeTransport returns a RoundTripper that is suitable for making requests
	// to external resources. The default implementation provides:
...
func MakeHTTPService(configService getConfig) HTTPService {
	return &HTTPServiceImpl{configService, RequestTimeout}
}
...
func (h *HTTPServiceImpl) MakeTransport(trustURLs bool) *MattermostTransport {
	...
	allowIP := func(ip net.IP) bool {
		...
		// In the case it's the self-assigned IP, enforce that it needs to be explicitly
		// added to the AllowedUntrustedInternalConnections
		for _, allowed := range strings.FieldsFunc(*h.configService.Config().ServiceSettings.AllowedUntrustedInternalConnections, splitFields) {

...
// IsReservedIP checks whether the target IP belongs to reserved IP address
// ranges to avoid SSRF attacks to the internal network of the Mattermost server
...
// IsOwnIP handles the special case that a request might be made to the public IP of
// the host which on Linux is routed directly via the loopback IP to any listening
// sockets, effectively bypassing host-based firewalls such as firewalld

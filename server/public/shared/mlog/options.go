...
// MaxQueueSize is the maximum number of log records that can be queued. If exceeded,
// `OnQueueFull` is called which determines if the log record will be dropped or block
// until add is successful. Defaults to DefaultMaxQueueSize.
...
// OnQueueFull, when not nil, is called on an attempt to add a log record to a full
// Logr queue. `MaxQueueSize` can be used to modify the maximum queue size.
// This function should return quickly, with a bool indicating whether the log
// record should be dropped (true) or block until the log record is successfully
// added (false). If nil then blocking (false) is assumed.
func OnQueueFull(f func(rec *LogRec, maxQueueSize int) bool) Option {
	return logr.OnQueueFull(f)
}

// OnTargetQueueFull, when not nil, is called on an attempt to add a log record to
// a full target queue provided the target supports reporting this condition.
...
// `updateFreqMillis` determines how often polled metrics are updated. Defaults to 15000
// (15 seconds) and must be at least 250 so we don't peg the CPU.
...
// MaxFieldLen is the maximum number of characters for a field. If exceeded,
// remaining bytes will be discarded. Defaults to DefaultMaxFieldLength.

...
type (
	...
	Option           = logr.Option
	Target           = logr.Target
	TargetInfo       = logr.TargetInfo
	LogRec           = logr.LogRec
	LogCloner        = logr.LogCloner
	MetricsCollector = logr.MetricsCollector
	TargetCfg        = logrcfg.TargetCfg
	TargetFactory    = logrcfg.TargetFactory
	FormatterFactory = logrcfg.FormatterFactory
	Factories        = logrcfg.Factories
	Sugar            = logr.Sugar
)
...
// Any picks the best supported field type based on type of val. For best performance
// when passing a struct (or struct pointer), implement `logr.LogWriter` on the struct,
// otherwise reflection will be used to generate a string representation.
var Any = logr.Any
...
// these wrapper calls will be inlined as well. Logger should be abbreviated as `logger`.
type Logger struct {
	log        *logr.Logger
	lockConfig *int32
}
...
func NewLogger(options ...Option) (*Logger, error) {
	...
	return &Logger{log: &log, lockConfig: &lockConfig}, nil
}
...
//
// For each case JSON containing log targets is provided. Target
// name collisions are resolved using the following precedence:
//
//	cfgFile > cfgEscaped
//
// An optional set of factories can be provided which will be
// called to create any target types or formatters not built-in.
...
// IsLevelEnabled returns true only if at least one log target is configured to
// emit the specified log level. Use this check when gathering the log info may
// be expensive.
//
...
// StdLogWriter returns a writer that can be hooked up to the output of a golang standard
// logger anything written will be interpreted as log entries and passed to this logger.
...
// `f` should return true to delete the target, false to keep it. When removing a target,
// best effort is made to write any queued log records before closing, with ctx determining
// how much time can be spent in total. Note, keep the timeout short since this method
// blocks certain logging operations.

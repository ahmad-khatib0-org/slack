...
func TestCommonMarkReferenceStrings(t *testing.T) {
	// For the most part, we aim for CommonMark compliance with the features that we support.
	// We also support some GitHub flavored extensions.
	//
	// You can find most of the references used here: https://github.github.com/gfm/
	//
	// CommonMark handles leading tabs that aren't on 4-character boundaries differently,
	// so the following reference strings will fail. The current implementation is much
	// closer to our webapp's behavior though, so I'm leaving it as is for now. It doesn't
	// really impact anything we use this package for anyways.
	//

...
// Given a string with a w at the given position, tries to parse and return a
// range containing a www link. if one exists. If the text at the given position
// isn't a link, returns an empty string. Equivalent to www_match from the reference code.
...
// Given a string with a : at the given position, tried to parse and return a range
// containing a URL scheme if one exists. If the text around the given position isn't
// a link, returns an empty string. Equivalent to url_match from the reference code.
func parseURLAutolink(data string, position int) (Range, bool) {
	...
	// Ensure that the URL scheme is allowed and that at least one character
	// after the scheme is valid.
...
// Given a string starting with a URL, returns the number of valid characters that make up
// the URL's domain. Returns 0 if the string doesn't start with a domain name. allowShort
// determines whether or not the domain needs to contain a period to be considered valid.
// Equivalent to check_domain from the reference code.
func checkDomain(data string, allowShort bool) int {
	...
	// If allowShort isn't set, a valid domain just requires at least a single period.
	// Note that this logic isn't entirely necessary because we already know the string
	// starts with "www." when this is called from parseWWWAutolink
	if foundPeriod {
		return i
	}
	return 0
...
// Returns true if the provided link starts with a valid character for a domain name.
// Equivalent to is_valid_hostchar from the reference code.
...
// Removes any trailing characters such as punctuation or stray brackets that shouldn't
// be part of the link. Returns a new end position for the link. Equivalent to
// autolink_delim from the reference code.
func trimTrailingCharactersFromLink(markdown string, start int, end int) int {
	...
	for linkEnd > 0 {
		...
		if !canEndAutolink(c) {
			...
			// Only allow an autolink ending with a bracket if that bracket is part of a
			// matching pair of brackets. If there are more closing brackets than opening
			// ones, remove the extra bracket

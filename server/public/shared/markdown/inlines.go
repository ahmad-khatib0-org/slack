...
func (p *inlineParser) parseAutolink(c rune) bool {
	...
	if c == ':' {
		...
		// Since the current position is at the colon, we have to rewind
		// the parsing slightly so that we don't duplicate the URL scheme
		...
		if rewind != -1 {
			...
			if !ok {
				// This should never occur since parseURLAutolink will only return a non-empty
				// value when the previous text ends in a valid URL protocol which would mean
				// that the previous node is a Text node
				return false
			}

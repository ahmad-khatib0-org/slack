...
func TestCopyDir(t *testing.T) {
	...
	tempFile := "temp.txt"
	err = os.WriteFile(filepath.Join(srcDir, tempFile), []byte("test file"), 0o655)
	require.NoError(t, err)

	childDir := "child"
	err = os.Mkdir(filepath.Join(srcDir, childDir), 0o777)
	require.NoError(t, err)

	childTempFile := "childtemp.txt"
	err = os.WriteFile(filepath.Join(srcDir, childDir, childTempFile), []byte("test file"), 0o755)
	require.NoError(t, err)
	...
	assert.NoError(t, err)
	assert.Equal(t, uint32(0o655), uint32(stat.Mode()))
	...
	assert.NoError(t, err)
	assert.Equal(t, uint32(0o755), uint32(stat.Mode()))
	assert.False(t, stat.IsDir())
